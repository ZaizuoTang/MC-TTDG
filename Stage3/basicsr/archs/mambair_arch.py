...
from archs.quantize_tool import Q_tools
# from archs.quantize_tool import VectorQuantizer, CombineQuantBlock, Q_tools
...
class MambaIR(nn.Module):
    ...
    def forward_features(self, x, Class_gt = None):
        
        x_size = (x.shape[2], x.shape[3])
        x = self.patch_embed(x) # N,L,C  #展平之后，加上正则化层

        x = self.pos_drop(x)    #这里没有开，数值设置的为0

        loss_all = 0.


        if self.training:
            x, loss_all, loss_class = self.Q_tool.forward_train(x, x_size, Class_gt)
        else:
            if self.is_test==False:
                x, pre_c = self.Q_tool.forward_valid(x, x_size)
            else:
                x, pre_c = self.Q_tool.forward_test(x, x_size)
        ...
        #     if self.training:
        #         if i == self.Num_codebook_feature:
        #             x, loss_all, loss_class = self.Q_tool.forward_train(x, x_size, Class_gt)
        #     else:
        #         if i == self.Num_codebook_feature:
        #             x, pre_c = self.Q_tool.forward_test(x, x_size)
        ...
        x = self.norm(x)  # b seq_len c #超分辨率还是使用了正则化的哦！！！
        x = self.patch_unembed(x, x_size)

        if self.training:
            return x, loss_all, loss_class
        else:
            return x, pre_c
    ...
    def forward(self, x, Class_gt=None):
        ...
        if self.training:
            temp, loss_codebook, loss_class = self.forward_features(x, Class_gt)
        else:
            temp, pre_class = self.forward_features(x)

        x = self.conv_after_body(temp) + x
        ...
        if self.training:
            return x, loss_codebook, loss_class
        else:
            return x, pre_class
