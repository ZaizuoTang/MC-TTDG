...
class MambaIR(nn.Module):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if self.training:
                if i == self.Num_codebook_feature:
                    x, loss_all = self.Q_tool.forward_train(x, x_size, Class_gt)
            else:
                if i == self.Num_codebook_feature:
                    x = self.Q_tool.forward_test(x, x_size)
    ...
    def forward(self, x, Class_gt=None):
        ...
        if self.training:
            temp, loss_codebook = self.forward_features(x, Class_gt)
        else:
            temp = self.forward_features(x)
